...
class CSVDailyBarDataSource(object):
    ...
    def _convert_bar_frame_into_bid_ask_df(self, bar_df):
        ...
        seq_oc_df = oc_df.T.unstack(level=0).reset_index() # MARK: unstack如何操作的
        seq_oc_df.columns = ['Date', 'Market', 'Price']
        seq_oc_df.loc[seq_oc_df['Market'] == 'Open', 'Date'] += pd.Timedelta(hours=9, minutes=30) # TODO: 修改交易时间
        seq_oc_df.loc[seq_oc_df['Market'] == 'Close', 'Date'] += pd.Timedelta(hours=15, minutes=00)
